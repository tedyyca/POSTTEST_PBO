...
1. **Static Polymorphism (Overloading)**:
    * Fungsi `setHarga()` pada class `Sepatu` memiliki dua versi: input harga langsung dan input harga dengan diskon persen.
2. **Dynamic Polymorphism (Overriding)**:
    * Method `tampilkan()` di-override oleh setiap subclass untuk menampilkan atribut spesifik secara otomatis.
...
---
*Laporan POSTTEST 4 Pemrograman Berorientasi Objek - Universitas Mulawarman.*